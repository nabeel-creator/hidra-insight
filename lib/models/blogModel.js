...
const BlogSchema = new mongoose.Schema({
    ...
    category: {
        type: String,
        required: false,
        // FIXED: Added 'General' to the enum and made it more flexible
        enum: ['Technology', 'Lifestyle', 'Business', 'Health', 'Travel', 'Food', 'Fashion', 'Sports', 'Entertainment', 'General', 'Other'],
        default: 'General' // Set General as default
    },
    tags: [{
        type: String,
        trim: true
    ...
    // Publication status - FIXED: Added both fields for compatibility
    status: {
        type: String,
...
BlogSchema.pre('save', function(next) {
    ...
    // Sync isPublished with status for consistency
    ...
    // Sync status with isPublished if status is not set
    if (!this.status) {
    ...
    if ((this.status === 'published' || this.isPublished) && !this.publishedAt) {
